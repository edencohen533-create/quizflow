...
export function trackingSandbox(key: string, containerId?: string): HTMLIFrameElement | undefined {
  ...
  frame.srcdoc = '<!doctype html><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src https: data:; script-src https: \'unsafe-inline\' \'unsafe-eval\'; connect-src https:; form-action \'none\'; base-uri \'none\'"><script>' +
    'window.dataLayer=[];const gtm=' + scriptJson(gtm) + ';' +
...
export function sendSandboxTracking(key: string, payload: { event?: Record<string, unknown>; code?: string }, containerId?: string) {
  const existing = frames.get(key);
  const frame = trackingSandbox(key, containerId);
  if (!frame) return;
