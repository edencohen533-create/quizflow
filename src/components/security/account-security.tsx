...
export function AccountSecurity(){
 ...
 return <section className="rounded-xl border p-5 space-y-3"><h2 className="font-semibold">אימות דו־שלבי</h2><p>{enabled?"אימות באפליקציית אימות פעיל בחשבון":"הוסיפו קוד מאפליקציית אימות להגנה על החשבון."}</p>{!enabled&&!factor&&<Button disabled={busy} onClick={enroll}>הפעלת אימות דו־שלבי</Button>}{factor&&<form onSubmit={verify} className="space-y-3">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={factor.qr} alt="קוד לסריקה באפליקציית אימות" width={200} height={200}/><Input aria-label="קוד אימות" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} required value={code} onChange={e=>setCode(e.target.value)}/><Button type="submit" disabled={busy}>אימות והפעלה</Button></form>}<p role="status">{message}</p></section>;
}
